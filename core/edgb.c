...
void edgb_KS_func(double X[NDIM], double edgb_KS[NDIM][NDIM])
{
  ...
  edgb_KS[2][2] = rho2
   ...
   + 187446.*pow(r,7))*a2*(1. - 3.*c2t))/(110250.*pow(r,8)) + (ep4*(45715050.*pow(r,11)*(-1. + 3.*c2t) + 5625.*pow(r,10)*(-20749. + 58131.*c2t) 
...
void edgb_trans(double r, double th, double edgb_T[NDIM][NDIM]) { 
  ...
  edgb_T[0][1] = 2.*r/(r*r - 2.*r + a*a) + zeta*(-1./30.*(-2240. + 144.*r + 96.*pow(r,2.) + 390.*pow(r,3.) + 20.*pow(r,4.) + 15.*pow(r,5.))/
...
// ROW 3 ENDS HERE
   ...
   edgb_T[3][1] = a/(r*r - 2.*r + a*a) + zeta*(-1./36750.*((70560000. - 96902400.*r + 57429400.*pow(r,2.) - 11130000.*pow(r,3.) + 4106590.*pow(r,4.) - 4567120.*pow(r,5.) + 76674.*pow(r,6.) + 
